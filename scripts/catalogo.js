...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function applyFilters() {
    ...
    const filteredPlatos = platosData.filter((plato) => {
      const matchesCategory =
        ...
        plato.filtroCategoria === selectedCategory;
      const matchesType =
        selectedType === "all" || plato.filtroOrden === selectedType;
      const matchesVegan =
        selectedVegan === "all" || plato.filtroVegano === selectedVegan;
  ...
  function initializeCounters() {
    ...
    counters.forEach((counter) => {
      ...
      const productImage = counter
        .closest(".productDetail")
        .querySelector(".img-plato img")
        .getAttribute("src");
      const productName = counter
        .closest(".productDetail")
        .querySelector(".titulo h4").textContent;
